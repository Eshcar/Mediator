...
mediator = load('mediator_throughput_native_read_u10.txt');
omid = load('omid_throughput_native_read_u10.txt');

% colormap(gray);

%diff = log2(mediator ./ omid);
diff = (mediator - omid) ./ omid;
...
title('(Mediator-Omid)/Omid throughput ratio');
h = get(gca, 'title');
set(h, 'FontSize', 24);
...
print -depsc mesh_color_u10.eps
